# Дизайн-система «Aurora» — манифест
...
> **Статус: это манифест.** Единственный источник истины для внешнего вида всего
> сервиса UWU (торговый интерфейс, админка, клиентский сайт `/shop`, страница
> входа). Любая правка UI обязана следовать правилам ниже; исключения — только
> осознанные и задокументированные.

## Манифест (обязательные правила)

### MUST

1. **Тёмная тема — по умолчанию** (`:root`). Светлая — вторичная
   (`[data-theme="light"]`). Ни одна страница не должна требовать светлой темы.
2. **Только токены.** В шаблонах и стилях компонентов запрещены «магические»
   цвета и размеры — только CSS-переменные из `:root` (иначе ломается тема).
3. **Фон «аврора»** (`.aurora`) присутствует на всех страницах (`base.html`,
   `login.html`, `customer/base.html`).
4. **Стеклянные панели.** Карточки/панели используют полупрозрачный `--surface`
   + `backdrop-filter: blur`.
5. **Градиентный акцент** `--accent-grad` (синий → бирюзовый) — для первичных
   действий (кнопка `.primary`, активные чипы/табы, бренд, FAB).
6. **Пилюльная геометрия** для кнопок, чипов, табов, бейджей (radius 999px).
7. **Иконки — только через `icon()`** из [`app/web/icons.py`](../app/web/icons.py);
   не дублировать SVG в шаблонах.
8. **Макросы.** Повторяющиеся блоки (модалка, поиск, бейдж, пустое состояние) —
   через [`_components.html`](../app/templates/_components.html), не копипастом.
9. **Анимации** плавные (`--ease`, `--ease-spring`) и уважают
   `prefers-reduced-motion`.
10. **Цвета графиков** — из системной палитры (`--blue … --gray`), перекрашиваются
    при смене темы (событие `themechange`).

### MUST NOT

- ❌ Светлые «белые» карточки на белом фоне в стиле Apple.
- ❌ Синий акцент `#0071e3` / нейтральный графит как основной.
- ❌ Инлайн-стили с явными цветами (`style="color:#333"` и т.п.).
- ❌ Новые иконки эмодзи/юникодом вместо `icon()`.
- ❌ Анимации без учёта `prefers-reduced-motion`.
- ❌ Дублирование разметки модалок/форм/поиска вместо макросов.
