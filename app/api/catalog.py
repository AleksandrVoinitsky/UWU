...
Все эндпоинты требуют аутентификацию и соответствующее право: чтение —
``catalog.read``, создание — ``catalog.write``. Константы (PUT) — ``service.settings``.
...
# Зависимости прав доступа.
READ = Depends(require_permission("catalog.read"))
WRITE = Depends(require_permission("catalog.write"))


@router.get("/valyuty", response_model=list[schemas.ValyutaOut])
async def list_valyuty(
...
async def create_kontragent(
    payload: schemas.KontragentBase,
    session: AsyncSession = Depends(get_session),
    _user: User = WRITE,
):
    if not payload.code:
...
async def create_dogovor(
    payload: schemas.DogovorBase,
    session: AsyncSession = Depends(get_session),
    _user: User = WRITE,
):
...
async def create_nomenklatura(
    payload: schemas.NomenklaturaBase,
    session: AsyncSession = Depends(get_session),
    _user: User = WRITE,
):
    if not payload.code:
...
async def set_constant(
    ...
    value: dict,
    session: AsyncSession = Depends(get_session),
    user: User = Depends(require_permission("service.settings")),
):
