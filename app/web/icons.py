"""Набор иконок интерфейса (inline SVG, стиль SF Symbols / Lucide).

Иконки заданы декларативно как словарь ``имя -> внутренний SVG`` и рендерятся
функцией :func:`icon` в ``markupsafe.Markup``. Один источник истины — не
дублируется в шаблонах, легко тестируется и расширяется.

См. также: :mod:`app.templates`, :mod:`app.web.helpers`.
"""
from __future__ import annotations

from markupsafe import Markup

# Внутренняя разметка (paths) каждой иконки в viewBox="0 0 24 24".
ICONS: dict[str, str] = {
    ...
    "sparkles": '<path d="M12 3v3M12 18v3M3 12h3M18 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M18.4 5.6l-2.1 2.1M7.7 16.3l-2.1 2.1"/>',
...
def icon(name: str, size: int = 18, cls: str = "") -> Markup:
    """Рендерит иконку по имени в inline-SVG.

    Неизвестное имя возвращает пустую строку (без исключения) — интерфейс
    деградирует мягко, а не падает на опечатке.
    """
    body = ICONS.get(name)
    if body is None:
        return Markup("")
    ...
    return Markup(
        f'<svg class="ico{cls_attr}" width="{size}" height="{size}" '
